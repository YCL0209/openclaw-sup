...
POST /api/auth/login
{
  ...
  "password": "bot-password"
         ...
         ↓
步骤 2: ERP authService.ts 處理
├─ 查找租户 (Tenant.findOne by taxId)
├─ 查找用户 (User.findOne by email + tenantId)
├─ 驗證密码 (user.comparePassword)
├─ 检查會話限制 (maxConcurrentSessions: 3)
└─ 生成 JWT Token
         ↓
步骤 3: ERP 返回登入響應
{
...
響應 (HTTP 401): Token 過期或无效
{
  "success": false,
  "message": "Token 已過期或无效"
}
```
...
### JWT 和會話配置（来自 sessionConfig.ts）

```javascript
// .env 中的配置
JWT_SECRET = 'YOUR_JWT_SECRET'
JWT_EXPIRES_IN = '8h'                              // 8 小时
TOKEN_GRACE_PERIOD_MINUTES = 30                    // 寬限期 30 分钟
MAX_CONCURRENT_SESSIONS = 3                        // 最多 3 个同时登入
SESSION_TIMEOUT_MINUTES = 480                      // 會話超时 8 小时
...
```
Token 生成 → Token 過期（8 小时后）→ 寬限期（30 分钟）→ 最終失效
...
// - 如果 Token 快過期（< 1 分钟）→ 调用 refreshToken
// - 否则 → 返回现有 Token
...
| **Token 有效期** | 8 小时（自动续期） |
| **寬限期** | 30 分钟（Token 過期后仍可刷新） |
| **最大并发會話** | 3 个 |
| **API 基址** | `http://localhost:3000` |
| **主要 API** | `/api/customers`, `/api/orders` |
| **错误處理** | 401 Token 无效，400 驗證失败，500 服务器错误 |
